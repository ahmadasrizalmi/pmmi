...
export function hermesBuildCommands(profileName:string,workspacePath:string){
  ...
  if(workerConfig.HERMES_START_GATEWAY)commands.push([workerConfig.HERMES_BIN,'-p',profileName,'gateway','start']);
  return commands;
}
...
export async function handleHermesEvent(event:OutboxEvent){
  ...
  if(event.topic==='hermes.profile.build'){
    ...
    try{
      ...
      await workerTx(async client=>{await client.query(`update hermes_profiles set status='READY',last_error=null,updated_at=now() where id=$1`,[profileId]);await client.query(`update hermes_workspaces set status='READY',updated_at=now() where profile_id=$1`,[profileId]);await client.query(`update hermes_build_jobs set status='SUCCEEDED',finished_at=now(),last_error=null where id=$1`,[jobId]);});
  ...
  if(event.topic==='hermes.profile.archive'){
    ...
    if(workerConfig.HERMES_ENABLED){try{await runCommand([workerConfig.HERMES_BIN,'-p',profile.profile_name,'gateway','stop']);}catch{/* gateway may already be stopped */}}
  ...
  if(event.topic==='hermes.user.archive'){
    ...
    for(const profile of profiles){if(workerConfig.HERMES_ENABLED){try{await runCommand([workerConfig.HERMES_BIN,'-p',profile.profile_name,'gateway','stop']);}catch{}}await workerTx(async client=>{await client.query(`update hermes_profiles set status='ARCHIVED',archived_at=now(),updated_at=now() where id=$1`,[profile.id]);await client.query(`update hermes_workspaces set status='ARCHIVED',archived_at=now(),updated_at=now() where profile_id=$1`,[profile.id]);});}
  }
}
