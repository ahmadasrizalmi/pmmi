...
async function removeExpired(table:'submission_upload_intents'|'application_document_upload_intents',limit=100){
  const client=clientFromEnv();if(!client)return 0;
  const rows=await workerPool.query(`select id,bucket,object_key from ${table} where consumed_at is null and expires_at<now() order by expires_at limit $1`,[limit]);
  let removed=0;
  for(const row of rows.rows){
    ...
    await workerPool.query(`delete from ${table} where id=$1 and consumed_at is null`,[row.id]);removed++;
  }
  return removed;
}
...
export async function cleanupExpiredUploads(force=false){
  ...
  const applications=await removeExpired('application_document_upload_intents');
