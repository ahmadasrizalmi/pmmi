...
test('PMMI blueprint end-to-end API flow',async()=>{
  ...
  await pool.query(`insert into ai_model_permissions(role,model_pattern,enabled) values('ADMIN','*',true),('USTADZ','*',true),('SANTRI','*',true) on conflict(role,model_pattern) do update set enabled=true`);
  ...
  try{
    ...
    const applicantHeaders={'x-applicant-token':application.applicantToken};
    ...
    response=await post(app,'/v1/ai/chat/completions',{model:'test/model',messages:[{role:'user',content:'Halo PMMI'}],stream:false,max_tokens:100},studentAuth);assert.equal(response.statusCode,200,response.body);assert.equal(json(response).choices[0].message.content,'Jawaban AI PMMI');
    ...
    const outbox=await pool.query(`select count(*)::int count from outbox_events where processed_at is null`);assert.ok(outbox.rows[0].count>5);
