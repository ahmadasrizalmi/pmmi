...
test('PMMI blueprint end-to-end API flow',async()=>{
  ...
  try{
    ...
    // Password reset flow
    response=await post(app,'/v1/auth/forgot-password',{email:'santri@example.com'});assert.equal(response.statusCode,200,response.body);assert.ok(json(response).ok);
    const resetEvent=await pool.query(`select payload from outbox_events where topic='auth.password_reset_requested' limit 1`);assert.ok(resetEvent.rowCount);const resetToken=resetEvent.rows[0].payload.token;
    response=await post(app,'/v1/auth/reset-password',{token:resetToken,password:'SantriNewPass!'});assert.equal(response.statusCode,200,response.body);assert.ok(json(response).reset);
    response=await post(app,'/v1/auth/login',{email:'santri@example.com',password:'SantriNewPass!'});assert.equal(response.statusCode,200,response.body);
    response=await post(app,'/v1/auth/login',{email:'santri@example.com',password:'Santri12345!'});assert.equal(response.statusCode,401,response.body);
    // Reset with used token fails
    response=await post(app,'/v1/auth/reset-password',{token:resetToken,password:'AnotherPass!'});assert.equal(response.statusCode,400,response.body);
    // Forgot password for unknown email is silent
    response=await post(app,'/v1/auth/forgot-password',{email:'nobody@example.com'});assert.equal(response.statusCode,200,response.body);assert.ok(json(response).ok);
    ...
    response=await post(app,'/v1/admissions/applications',{admissionPeriodId:periodId,applicantName:'Alumni Test',email:'alumni@example.com',phone:'628111111111'});const app2=json(response);for(const status of ['ADMIN_VERIFIED','SCREENING','ACCEPTED']){response=await app.inject({method:'PATCH',url:`/v1/admissions/applications/${app2.id}/status`,headers:admin,payload:{status}});assert.equal(response.statusCode,200,response.body);}response=await app.inject({method:'PUT',url:`/v1/admissions/applications/${app2.id}/registration`,headers:{'x-applicant-token':app2.applicantToken},payload:{programId,cohortId,metadata:{}}});assert.equal(response.statusCode,200,response.body);response=await app.inject({method:'PATCH',url:`/v1/admissions/applications/${app2.id}/status`,headers:admin,payload:{status:'ENROLLED'}});assert.equal(response.statusCode,200,response.body);const enrolled2=json(response);assert.ok(enrolled2.userId,'enrollment should return userId');const student2=await pool.query(`select id from students where user_id=$1`,[enrolled2.userId]);assert.ok(student2.rowCount,'student record should exist');
    // GRADUATED maps to GRADUATED_PENDING with scheduled ALUMNI transition
    response=await app.inject({method:'PATCH',url:`/v1/students/${student2.rows[0].id}/status`,headers:admin,payload:{status:'GRADUATED'}});assert.equal(response.statusCode,200,response.body);assert.equal(json(response).status,'GRADUATED_PENDING');
    const wallet2=await pool.query(`select balance from ai_credit_wallets where user_id=$1`,[enrolled2.userId]);assert.equal(Number(wallet2.rows[0].balance),0);
    const graduatedPendingEvent=await pool.query(`select available_at from outbox_events where topic='student.lifecycle.graduated_pending_expired' and aggregate_id=$1 limit 1`,[student2.rows[0].id]);assert.ok(graduatedPendingEvent.rowCount,'graduated pending event should be scheduled');assert.ok(new Date(graduatedPendingEvent.rows[0].available_at)>new Date());
    // Direct ALUMNI transition (bypass grace period)
    response=await app.inject({method:'PATCH',url:`/v1/students/${student2.rows[0].id}/status`,headers:admin,payload:{status:'ALUMNI'}});assert.equal(response.statusCode,200,response.body);assert.equal(json(response).status,'ALUMNI');
