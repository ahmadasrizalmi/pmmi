...
test('completion surfaces are enforceable at API runtime',async()=>{
  ...
  try{
    ...
    response=await app.inject({method:'OPTIONS',url:'/v1',headers:{origin:'https://app.pondokmultimedia.id','access-control-request-method':'GET'}});assert.ok(response.statusCode<400,response.body);assert.equal(response.headers['access-control-allow-origin'],'https://app.pondokmultimedia.id');
    response=await app.inject({method:'OPTIONS',url:'/v1',headers:{origin:'https://evil.example','access-control-request-method':'GET'}});assert.notEqual(response.headers['access-control-allow-origin'],'https://evil.example');assert.ok(response.statusCode>=400,'untrusted origin should be rejected');
