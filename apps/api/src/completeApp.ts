...
export async function buildCompleteApp(){
  const app=Fastify({logger:true});
  await app.register(cors,{origin:true});
  await app.register(jwt,{secret:config.JWT_SECRET});
  app.get('/health',async()=>({status:'ok',service:'pmmi-api',runtime:'complete-blueprint'}));
  app.get('/v1',async()=>({name:'PMMI Digital Campus API',status:'complete-blueprint',corePhases:[1,2]}));
